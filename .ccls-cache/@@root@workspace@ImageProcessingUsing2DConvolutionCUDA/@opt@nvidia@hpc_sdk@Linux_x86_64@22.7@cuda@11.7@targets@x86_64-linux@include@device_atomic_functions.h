...
#elif defined(__PGI_CUDA__)
/* PGI CUDA C++ lili2cuda bridge needs to know if the callee is a builtin
 * function. If so, the code generation is able to typecast and make paremeter
 * types exactly match the prototype of each builtin function. */
...
/* Add !defined(__PGI_CUDA__) to avoid empty function definition in PGI CUDA
